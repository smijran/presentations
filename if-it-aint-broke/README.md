...
>
> **The Scalability Cliff — JDK 23:** `JDK-8180450` ("secondary_super_cache does not scale well") — a 20-year-old HotSpot design flaw, fixed with no JEP. HotSpot caches `instanceof`/`checkcast` results against interfaces in one shared field per class; under multi-threaded contention with varied interface types, concurrent updates to that field cause false sharing / cache-line ping-ponging between cores. Hit real frameworks: **Netty, Vert.x, Mutiny, Hibernate, Quarkus** — Netty even filed it as its own GitHub issue (netty/netty#12708) for `HttpObjectEncoder`. Fix: an update-count counter that skips the cache update under detected contention, falling back to a linear scan instead. Confirmed Fix Version = 23, no JEP.
>
> **The numbers:** `SecondarySuperCacheInterContention` 1873.5 ns/op → 8.2 ns/op (**~228x**); `SecondarySuperCacheIntraContention` ~2780 ns/op → ~8 ns/op (**~347x**). Sourced from actual JMH output in a later platform-port PR (#22341, s390x) that reuses the same benchmark this fix family introduced — the original JDK 23 PR (#18309, C2/x86/aarch64) itself only gives qualitative notes ("immeasurably small... down in the noise" on Renaissance/SPECjvm), so the concrete multiplier comes from the platform-port validation using the identical benchmark. Companion tickets extending the same fix to other tiers/platforms all landed in JDK 24: `JDK-8331341` (C1/interpreter) plus PPC64/s390x/RISC-V ports — this is the same platform-port story the JDK 24 examples above reference. **Caveat, same rigor as elsewhere:** general-purpose benchmarks showed no broad change — this fixes a scalability cliff under contention, not a universal speedup.
>
...
> **Real-world proof:** Elastic rewrote Apache Lucene's vector-similarity dot product with the Vector API — the same technique as the demo above, shipped in production. The change was significant enough that Elasticsearch's own nightly benchmark jumped overnight — enough that the benchmarking team messaged the engineer on Slack asking if something had broken. It hadn't; it just got faster. Source: Ben Trent (Elastic), "Java Must Be Faster: Time for the Vector API," Carolina Code Conference 2023 — Trent worked directly on the Lucene integration and also cites an ~8x throughput speedup on a 256-bit-vector micro-benchmark.
...
| **Compact Object Headers** | ~10–20% less heap (JEP 450/519 literature estimate — `project_liliput`'s own demo is a 100-object toy app, not a real measured workload) | Only helps object-heavy workloads; benefit scales with object *count*, not size — collection/DTO/cache-heavy services gain more than array/primitive-heavy ones | ~5–15 machines | $690–$2,080 | $8,320–$24,970 |
...
**Not simply additive** — a given machine doesn't get all five benefits at once; they apply to different workload slices of a real fleet. A blended, non-double-counted estimate for a mixed 100-machine fleet: roughly **25–40 machines reclaimed → ~$3,500–$5,500/month, or ~$42,000–$66,000/year** — from opt-in features already sitting in the JDK, zero new hardware, and (for four of the five) zero app rewrites. *(This blended total predates the Compact Object Headers row above and hasn't been recomputed to fold it in — treat it as illustrative, not updated math.)*

Lead with **Virtual Threads** — "scaled out because platform threads ran out" is an extremely common real-world Java microservice pattern, making it the most defensible headline number. Vector API and Compact Strings are real but workload-specific — say so explicitly on the slide, don't let "4.53x" or "45x" get quoted out of context as a general fleet number. GC and Compact Object Headers are the softest claims (reclaimed headroom / literature estimate, not measured savings) — flag both as assumptions, not facts.
